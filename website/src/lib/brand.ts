...
export async function getHomepageHero(): Promise<BrandHero | null> {
  ...
  // FALLBACK: Try static media.v1.json if KV assignment failed
  ...
  if (staticMediaId) {
    try {
      ...
      if (staticMedia && staticMedia.lifecycleState === 'published' && staticMedia.source === 'local') {
        console.log('[PUBLIC_READER] STATIC_MEDIA_FOUND', { 
          requestId, 
      ...
      console.error('[PUBLIC_READER] STATIC_MEDIA_LOAD_FAILED', { requestId, error });
    }
  }
...
export async function getOwnerPortrait(): Promise<BrandOwnerPortrait | null> {
  ...
  // FALLBACK: Try static media.v1.json if KV assignment failed
  // This is temporary until KV is bootstrapped with media records
  console.log('[BRAND] FALLBACK_TO_STATIC', { requestId });
  ...
  if (staticMediaId) {
    try {
      ...
      if (staticMedia && staticMedia.lifecycleState === 'published' && staticMedia.source === 'local') {
        console.log('[BRAND] STATIC_MEDIA_FOUND', { 
          requestId, 
          mediaId: staticMediaId,
          lifecycleState: staticMedia.lifecycleState,
          source: staticMedia.source
      ...
      console.error('[BRAND] STATIC_MEDIA_LOAD_FAILED', { requestId, error });
    }
  }
