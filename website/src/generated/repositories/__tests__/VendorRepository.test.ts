/**
 * VendorRepository — generated tests.
 *
...
describe("VendorRepository (generated)", () => {
  ...
  it("afterSave is a no-op (stub)", async () => {
    await expect(
      repo.afterSave({ id: "test", version: 0, status: "initial", authority: "VendorAuthority", events: [] }, []),
    ).resolves.toBeUndefined();
  });

  it("snapshot returns same state (stub)", async () => {
    const state = { id: "test", version: 0, status: "initial", authority: "VendorAuthority", events: [] as never[] };
