/**
 * VoiceRepository — generated tests.
 *
...
describe("VoiceRepository (generated)", () => {
  let repo: VoiceRepository;

  beforeEach(() => {
    repo = new VoiceRepository();
  });
  ...
  it("afterSave is a no-op (stub)", async () => {
    await expect(
      repo.afterSave({ id: "test", version: 0, status: "initial", authority: "VoiceAuthority", events: [] }, []),
    ).resolves.toBeUndefined();
  });

  it("snapshot returns same state (stub)", async () => {
    const state = { id: "test", version: 0, status: "initial", authority: "VoiceAuthority", events: [] as never[] };
