...
export class EventConsumer {
  ...
  private async handleEstimateSent(event: DomainEvent): Promise<void> {
    this.recordMetric(AutomationMetricEventType.REQUEST, event.timestamp);
  ...
  private async handleProjectBooked(event: DomainEvent): Promise<void> {
    ...
    if (workflowTemplateId) {
      ...
      this.recordMetric(AutomationMetricEventType.WORKFLOW_STARTED, event.timestamp);
      
      try {
        await this.workflowEngine.executeWorkflow(workflowTemplateId, context);
        this.recordMetric(AutomationMetricEventType.WORKFLOW_COMPLETED, event.timestamp);
      } catch (error) {
        this.recordMetric(AutomationMetricEventType.WORKFLOW_FAILED, event.timestamp);
