...
export function SectionHeading({
  ...
  return (
    <div className={cn("max-w-2xl", align === "center" && "mx-auto text-center")}>
      ...
      <h2 className="mt-5 font-display text-4xl font-bold leading-[1.08] tracking-tight text-text sm:text-5xl">{title}</h2>
      {description && <p className="mt-6 text-lg text-text-muted sm:text-xl leading-relaxed">{description}</p>}
    </div>
  );
}
